...
export default function FloatingBuy(){
  ...
  console.log("[FloatingBuy] on", pathname);

  // derive propertyId; default to kin-001 if not on /property/*
  const slug = pathname.split("/").pop() || "";
  const map: Record<string,string> = { "kin-nsele":"kin-001" };
  const propertyId = (pathname.startsWith("/property/") ? (map[slug] ?? slug) : "kin-001") || "kin-001";
