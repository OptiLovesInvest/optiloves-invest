...
async function getData(): Promise<Res[]> {
  try {
    const res = await fetch(`${(process.env.NEXT_PUBLIC_BACKEND ?? (process.env.NEXT_PUBLIC_BACKEND ?? process.env.NEXT_PUBLIC_BACKEND_URL)_URL)?.replace(/\/$/, "")}/reservations`, { cache: "no-store" });
